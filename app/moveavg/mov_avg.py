...
##       print("Sell {0} its close is Higher then 20 day moving average by {1}({2})".format(ticker,pricdiff,close[ends]))
    ...
    plt.plot(chgprice[starts:ends]['Price Chg'], label="Adj Close - 200 day MA".format(ticker), linewidth = 2) 
    plt.plot(chgprice[starts:ends]['SMA 15'], label='15 day MA', linewidth = 1.5)
    plt.plot(chgprice[starts:ends]['SMA 20'], label='20 day MA', linewidth = 1.5)
    plt.plot(chgprice[starts:ends]['SMA 50'], label='50 day MA', linewidth = 1.5)
    plt.plot(chgprice[starts:ends]['SMA 200'], label='200 day MA', linewidth = 1.5)
    plt.xlabel('Date')
