...
def get_dates():
    ...
    IEX_date = date_today - timedelta(days = 1) # 30 trailing calendar days acc. to IEX docu

    # Get last date for which download was done to get start date
    try:
        folder_date = []
        year = max([name for name in os.listdir('/data/intraday_stocks/') if not name.startswith('.')])
        foldernames = os.listdir('/data/intraday_stocks/{}'.format(year))
        folder_CW = [name for name in foldernames if not name.startswith('.')]

        for i in range(len(folder_CW)):
            foldernames = os.listdir('/data/intraday_stocks/{}/{}'.format(year, folder_CW[i]))
...
def get_path(date_str):
    ...
    path = '/data/intraday_stocks/{}/{}-CW{}/{}'.format(year, year, CW, date_str)
